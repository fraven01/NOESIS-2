...
def test_simulated_user_middleware_switches_identity(client, settings, monkeypatch):
    ...
    monkeypatch.setattr("theme.views._tenant_context_from_request", lambda r: ("dev-tenant", "dev-schema"))

    # Create users
    User = get_user_model()
    real_user = User.objects.create_user("real_user", "real@example.com", "password")
    simulated_user = User.objects.create_user("simulated_alice", "alice@example.com", "password")
...
def test_simulated_user_middleware_ignored_in_production(client, settings, monkeypatch):
    ...
    User = get_user_model()
    simulated_user = User.objects.create_user("simulated_bob", "bob@example.com", "password")
