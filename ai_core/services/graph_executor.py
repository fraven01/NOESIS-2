...
def execute_graph(request: Request, graph_runner: GraphRunner) -> Response:
    ...
    """
    normalize_meta = _resolve_service_callable("_normalize_meta", _normalize_meta)
    get_checkpointer = _resolve_service_callable("_get_checkpointer", _get_checkpointer)
    should_enqueue_graph = _resolve_service_callable(
        "_should_enqueue_graph", _should_enqueue_graph
    )
    with_scope_apply_async_fn = _resolve_service_callable(
        "with_scope_apply_async", with_scope_apply_async
    )
    update_observation_fn = _resolve_service_callable(
        "update_observation", update_observation
    )
    emit_event_fn = _resolve_service_callable("emit_event", emit_event)
    lf_tracing_enabled_fn = _resolve_service_callable(
        "lf_tracing_enabled", lf_tracing_enabled
    )
    lf_start_trace_fn = _resolve_service_callable("lf_start_trace", lf_start_trace)
    lf_end_trace_fn = _resolve_service_callable("lf_end_trace", lf_end_trace)
    try:
        services = import_module("ai_core.services")
