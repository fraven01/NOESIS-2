...
class Command(BaseCommand):
    ...
    def _fail_duplicate_slug(self, model: str, slug: str):
        payload = {
            "event": "check.failed",
    ...
    def _emit_failure(self, payload: Dict[str, object]):
        self.stdout.write(json.dumps(payload, ensure_ascii=False))
        raise SystemExit(1)
    ...
    def _validate_projects_and_documents(
        ...
        if duplicate_project_slugs:
            slug = sorted(set(duplicate_project_slugs))[0]
            self._fail_duplicate_slug("Project", slug)
        ...
        for project_slug, documents in project_documents.items():
            for document in documents:
                ...
                if meta.get("invalid") is True or has_issues:
                    invalid_documents += 1

        if duplicate_document_slugs:
            slug = sorted(set(duplicate_document_slugs))[0]
            self._fail_duplicate_slug("Document", slug)
        ...
        if profile == "chaos" and invalid_documents == 0:
            self._fail("chaos_missing_invalids", {"expected_minimum": 1})
