...
def test_hybrid_search_applies_single_collection_filter(monkeypatch):
    ...
    monkeypatch.setattr(
        client,
        "_embed_query",
        lambda _q: [1.0] + [0.0] * (vector_client.get_embedding_dim() - 1),
    )
    ...
    fallback_queries = [
        ...
        if "select 1 from embeddings" in sql.lower()
    ]
...
def test_update_lifecycle_state_marks_documents_and_chunks():
    ...
    with client.connection() as conn, conn.cursor() as cur:
        ...
        assert chunk_meta["lifecycle_changed_at"].startswith(
