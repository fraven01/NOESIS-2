...
- [ ] **RAG memory scoping decision (checkpointer key)**: decide and implement memory scoping key for RAG threads (pointers: `ai_core/graph/core.py:GraphContext`, `ai_core/graph/core.py:FileCheckpointer._path`, `roadmap/rag_roadmap.md#L143`; acceptance: confirmed key shape using existing BusinessContext fields (tenant_id + case_id + collection_id + workflow_id), documented in roadmap, checkpointer uses the approved key, no new IDs/meta keys introduced; if key change occurs, add migration/reset note)
