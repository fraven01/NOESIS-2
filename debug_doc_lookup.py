        ...
        try:
            with schema_context(schema):
                ...
                if doc:
                    ...
                    # Check memberships
                    memberships = DocumentCollectionMembership.objects.filter(document_id=doc.id)
                    print(f"    Collection memberships: {memberships.count()}")
                    for m in memberships:
                        print(f"      - Collection: {m.collection.name if m.collection else 'N/A'} (key: {m.collection.key if m.collection else 'N/A'})")
                else:
