...
2. View ruft `assert_case_active` auf: löst Tenant, prüft Case (Auto-Create optional), blockiert geschlossene Cases.
3. Dispatcher erzeugt `run_id`, setzt `workflow_id` (`collection_search`) und legt Celery-Task `llm_worker.tasks.run_graph` in Queue `agents` mit Kontext (`tenant_id`, `case_id`, `workflow_id`, `trace_id`, `run_id`).
4. Graph erhält `GraphContextPayload`, validiert Pflichtfelder, propagiert IDs in jeden Tool-Call (`WebSearchWorker`, Hybrid-Score).
5. Tools erzeugen `invocation_id` und geben Outcomes mit identischen Kontextfeldern an Langfuse/Logs weiter.
