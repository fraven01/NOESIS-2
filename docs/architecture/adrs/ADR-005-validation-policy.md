...
## Entscheidung
- **Fail-fast**: `tenant_id`, `trace_id`, mindestens eine Laufzeit-ID (`run_id` und/oder `ingestion_run_id` - beide können koexistieren, z.B. wenn Workflow Ingestion triggert) sind immer Pflicht. `case_id` ist Pflicht für fachliche Workloads, optional für System-Tasks (Cron-Crawler, Backfill).
...
- **Validation-Ort**: HTTP-Views nutzen `assert_case_active`; Graph-Boundaries validieren via `GraphIOSpec` + `ToolContext`; Tools validieren Inputs vor externer API-Kommunikation.
