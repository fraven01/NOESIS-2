...
class FileBlob(BaseModel):
    ...
    sha256: str = Field(description="Hex encoded SHA-256 checksum of the blob contents.")
...
class InlineBlob(BaseModel):
    ...
    type: Literal["inline"] = Field(description="Discriminator identifying inline data blobs.")
...
class ExternalBlob(BaseModel):
    ...
    type: Literal["external"] = Field(description="Discriminator identifying an external blob reference.")
...
class DocumentMeta(BaseModel):
    ...
    @classmethod
    def _normalize_external_ref(cls, value: Optional[Dict[str, str]]) -> Optional[Dict[str, str]]:
        if value is None:
            return None
        if len(value) > 16:
...
class Asset(BaseModel):
    ...
    blob: BlobLocator = Field(description="Blob containing the asset payload or pointer.")
    origin_uri: Optional[str] = Field(
    ...
    caption_model: Optional[str] = Field(
        default=None, description="Model identifier if captions were generated by a VLM."
    )
    ...
    def _validate_relationships(self) -> "NormalizedDocument":
        ...
        if self.source == "integration":
            ...
            if any(key in external_ref for key in ("model", "generated_at", "topic")):
                ...
                iso_value = generated_at[:-1] + "+00:00" if generated_at.endswith("Z") else generated_at
                try:
