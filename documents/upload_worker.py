...
class UploadWorker:
    ...
    def process(
        ...
        *,
        ...
        state = self._compose_state(
            payload_path, checksum, size, tenant_id, 
            case_id, trace_id, resolved_document_id, 
            raw_meta, ingestion_overrides
        )
        
        # 7. Compose meta (IDENTICAL to Crawler!)
        meta = self._compose_meta(
            tenant_id, case_id, trace_id, invocation_id
        )
        ...
        blob_uri = state.get("normalized_document_input", {}).get("blob", {}).get("uri", "")
        ...
        return WorkerPublishResult(
            status="published",
            document_id=resolved_document_id or state["normalized_document_input"]["ref"]["document_id"],
    ...
    def _persist_payload(
        ...
        writer = ObjectStoreBlobWriter(
            prefix=[safe_tenant, safe_case, "uploads"]
        )
            ...
            with tenant_context(tenant):
                ...
                if collection_identifier:
                    ensured = self._ensure_collection_with_warning(
                        ...
                        embedding_profile=ingestion_overrides.get("embedding_profile") if ingestion_overrides else None,
                        scope=ingestion_overrides.get("scope") if ingestion_overrides else None,
                    )
                    if ensured:
    ...
    def _get_domain_service(self) -> DocumentDomainService:
        if self._domain_service is None:
            ...
            self._domain_service = DocumentDomainService(vector_store=get_default_client())
    ...
    def _ensure_collection_with_warning(
        ...
        try:
            ...
            return service.ensure_collection(
             ...
             logger.warning(
                 "upload_worker.collection_ensure_failed",
                 exc_info=True
             )
             return None
