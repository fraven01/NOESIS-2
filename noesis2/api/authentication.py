...
class LiteLLMMasterKeyAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request) -> Optional[Tuple[LiteLLMAdminUser, str]]:
        ...
        if len(auth) == 1:
            raise exceptions.AuthenticationFailed("Authorization header missing bearer token.")

        if len(auth) > 2:
            raise exceptions.AuthenticationFailed("Authorization header must not contain spaces in token.")

        try:
            provided_token = auth[1].decode()
        except UnicodeError as exc:  # pragma: no cover - defensive guard
            raise exceptions.AuthenticationFailed("Invalid bearer token encoding.") from exc

        master_key = getattr(settings, "LITELLM_MASTER_KEY", "")
        if not master_key:
            raise exceptions.AuthenticationFailed("LiteLLM master key is not configured.")
    ...
    def authenticate_header(self, request) -> str:  # pragma: no cover - mirrors DRF default behaviour
        return "Bearer"
