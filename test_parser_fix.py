...
def main():
    ...
    with schema_context(TENANT_SCHEMA):
        ...
        print(f"Blob media_type: {normalized_doc_dict.get('blob', {}).get('media_type')}")
        print(f"External ref media_type: {normalized_doc_dict.get('meta', {}).get('external_ref', {}).get('media_type')}")
        
        # Build a NormalizedDocument
        try:
            normalized_document = NormalizedDocument.model_validate(normalized_doc_dict)
            print(f"Loaded NormalizedDocument successfully")
            print(f"  blob.media_type: {getattr(normalized_document.blob, 'media_type', None)}")
